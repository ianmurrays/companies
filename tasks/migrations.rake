# @link https://github.com/lizconlan/sinatra-migrations
...
namespace :db do
  ...
  namespace :migrate do
    desc "Creates migration with a name (usage: rake db:migrate:create[name_of_the_migration]"
    task :create, [:name] => :environment do |t, args|
      File.open("db/migrate/#{Time.now.to_i}_#{args[:name]}.rb", "w") do |file|
        file.write <<-EOS
class #{args[:name].capitalize} < ActiveRecord::Migration
  def up
  end
